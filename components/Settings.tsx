...
const SettingsView: React.FC<Props> = ({ 
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-slide-in pb-24">
      {/* Profile Settings */}
      <section className={`p-8 rounded-[2.5rem] ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} border shadow-sm`}>
        ...
        <form onSubmit={handleSaveProfile} className="space-y-6">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className={`w-full pl-12 pr-4 py-4 rounded-2xl border-2 outline-none transition-all appearance-none ${
                    darkMode ? 'bg-slate-900 border-slate-700 focus:border-green-600' : 'bg-slate-50 border-slate-200 focus:border-green-600'
                  }`}
                >
          ...
          <button
            type="submit"
            className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white font-black px-8 py-4 rounded-2xl shadow-xl shadow-green-600/20 transition-all active:scale-95"
          >
      ...
      <section className={`p-8 rounded-[2.5rem] ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} border shadow-sm`}>
        ...
        <div className="space-y-4">
            ...
            <div className="flex bg-slate-200 dark:bg-slate-700 p-1 rounded-2xl">
              <button
                ...
                className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${
                  lang === 'en' ? 'bg-white dark:bg-slate-600 text-green-600 shadow-xl' : 'text-slate-500'
                }`}
              >
                EN
              </button>
              <button
                onClick={() => setLang('tw')}
                className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${
                  lang === 'tw' ? 'bg-white dark:bg-slate-600 text-green-600 shadow-xl' : 'text-slate-500'
                }`}
              >
                TW
              </button>
          ...
          <div className="flex items-center justify-between p-5 rounded-3xl bg-slate-50 dark:bg-slate-900/50">
            ...
            >
              <div className={`w-6 h-6 bg-white rounded-full absolute top-1 shadow-md transition-all ${
